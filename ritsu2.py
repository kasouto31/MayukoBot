...
prefix = "?"
bot = commands.Bot(command_prefix = f"{prefix}", description = "")
#-------------------------------------------------------------------------------------------
role = 'Fondateurs'
musics = {}
...
async def on_member_join(member):
    embed=discord.Embed(title="", description="", color=000000, timestamp=datetime.now()) #image1
    embed.set_author(name="{} vient de rejoindre le serveur".format(member.name), url="", icon_url="") #image2
    embed.set_footer(text="")
    channel1 = bot.get_channel(816713276993568862)
...
@bot.event
async def on_member_join(member): 
    rank = discord.utils.get(member.guild.roles, name=role3) 
...
async def on_ready():
    ...
    await bot.change_presence(game=discord.Game(name="?aide | ".format(len(bot.servers), len(set(bot.get_all_members()))), type=3))
...
async def on_guild_join(guild):
    ...
    for channel in guild.text_channels:
        if channel.permissions_for(guild.me).send_messages:
            await channel.send("Hey! Ritsu est en ligne")
        break
...
#-------------------------------------------------------------------------------------------
@bot.event
async def on_ready():           # affiche ready dans le terminale une fois le bot pret a être utiliser
    activity = discord.Game(name=f"{prefix}", type=2)
    await bot.change_presence(activity=discord.Streaming(name=f"{prefix}aide, venez visiter notre site", url="https://ritsu-shop.000webhostapp.com/"))
    print("ready ! Ritsu Principale")
...
#
#    channel = bot.get_channel(783632549079285770) # ID du channel
 #   await channel.send(f"""Bienvenue à {discord.Member.mention} sur le serveur !
  #  Nous somme désormais {numberOfPoeple} sur le serveur {serverName} merci à toi !
   # """)
 ...
 #   server = ctx.guild
  #  numberOfTextChannels = len(server.text_channels)
   # NumberOfVoiceChannels = len(server.voice_channels)
    ...
    #channel = bot.get_channel(783632549079285770) # ID du channel
    #await channel.send(f"""Au revoir à {discord.Member.mention} :(
    #Nous somme désormais {numberOfPoeple} sur le serveur {serverName} 
    #""")
...
# servere info command
@commands.check(botcheck)
@bot.command(pass_context=True)
async def serverinfo2(ctx):
...
async def genServerRp(ctx):
    ...
    #catégorie 6 
    await ctx.guild.create_voice_channel(f"exemple-fiche-📄", category=category6)
    await ctx.guild.create_voice_channel(f"📗-salon-fiche-1-📗", category=category6)
    await ctx.guild.create_voice_channel(f"📘-salon-fiche-2-📘", category=category6)
    await ctx.guild.create_voice_channel(f"📙-salon-fiche-3-📙", category=category6)
    await ctx.guild.create_voice_channel(f"📕-salon-fiche-4-📕", category=category6)
    #catégorie7
    await ctx.guild.create_voice_channel(f"portail-dentrée", category=category7)
    await ctx.guild.create_voice_channel(f"allée-devant-le-lycée", category=category7)
...
@bot.event
async def on_member_join(member):
    channel = bot.get_channel(816713276993568862)
...
bot.run("ODE1Mjg3MTI3ODE3ODQ2ODQ1.YDqNTg.Js_ZrMrGLTqOhagIxu5JPegwPSE") # permet de lancer le bot 

#NzA0MDEzNDg3Mjk4NTc2NDQ2.XqW9pw.-IlQsvgOE5v4Rj2LbYBDI6MrURU

#Nzk5NjMyMjExMTY2ODg3OTY2.YAGZhw.jcYi4OaFTFGUnYD_ig9p2HOgXsA token de test a ne pas sup fdp

#ODAyNjA5Njg5NjgxNDYxMjg4.YAxuhQ.69hKG2UK-fEYgW3kOwHNSM8OSD8 Ritsu TMT token 
